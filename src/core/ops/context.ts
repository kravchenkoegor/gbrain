...
import { CJK_SLUG_CHARS, SLUG_WORD_CHARS, SLUG_EXTRA_SCRIPT_CHARS } from '../cjk.ts';
...
export function validateFilename(name: string): void {
  ...
  // LOCAL PATCH (slug i18n): Cyrillic filenames too — validatePageSlug went
  // any-script in #3417, but this validator is still CJK-only upstream.
  const FILENAME_CHARS = `a-zA-Z0-9${CJK_SLUG_CHARS}${SLUG_EXTRA_SCRIPT_CHARS}`;
  const FILENAME_RE = new RegExp(`^[${FILENAME_CHARS}][${FILENAME_CHARS}._\\-]*$`);
