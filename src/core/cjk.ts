...
/**
 * LOCAL PATCH (slug i18n): Cyrillic block U+0400–U+04FF, kept separate from
 * the CJK constant above (which also drives countCJKAwareWords density — adding
 * Cyrillic there would char-tokenize and over-split Russian prose). Page slugs
 * went any-script upstream in #3417; this remains for validateFilename in
 * ops/context.ts, which is still CJK-only. Re-apply after `gbrain upgrade`.
 */
